...
async def judge_route_case(case: dict) -> dict:
    ...
    prompt = (
        ...
        "- If the request starts from a generic product or workflow scenario, or the owner layer is unresolved, include agently first when it is installed.\n"
        "- If the request explicitly says the owner layer is not decided yet, do not route past agently.\n"
        "- If the request explicitly says it has not decided between patching one request and upgrading to workflow orchestration, stop at agently even when the likely eventual owner could be agently-triggerflow.\n"
        "- If the request explicitly says it has not decided between helper chains, function chains, config cleanup, or workflow orchestration, stop at agently even when agently-triggerflow looks likely later.\n"
        ...
        "- Mentions of a UI, web page, desktop shell, or local model service such as Ollama are supporting constraints and do not disqualify agently when the request is still about building a model-powered tool.\n"
        "- If one installed non-playbook skill directly and narrowly owns the request, prefer that leaf skill without agently.\n"
        "- If the request is still unresolved between one request family and workflow orchestration, stop at agently.\n"
